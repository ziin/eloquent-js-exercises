...
// sort for recorrence
// computate all routes and choose the shortest one
// prefer pick up packages
function myRobot({ place, parcels }, routes) {
  if (routes.length === 0) {
    ...
    routes = routes.reduce((acc, cur) => (acc.length < cur.length ? acc : cur));
  }
  return { direction: routes[0], memory: routes.slice(1) };
}
